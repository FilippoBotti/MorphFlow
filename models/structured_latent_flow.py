...
from models.sparse_structure_flow import TimestepEmbedder
...
class SLatFlowModel(nn.Module):
    """
    TRELLIS SLat flow adapted for MorphFlow conditioning.

    Input and output are sparse tensors with fixed coords and dense features:
    coords [sumN, 4], feats [sumN, C]. The model predicts flow velocity on
    feats only; coordinate support is preserved.
    """
    ...
    def __init__(
        ...
        if self.io_block_channels is not None:
            ...
            assert np.log2(patch_size) == len(io_block_channels), "Number of IO stages must match patch_size"
        ...
        first_channels = model_channels if io_block_channels is None else io_block_channels[0]
        self.input_layer = sp.SparseLinear(in_channels, first_channels)
        ...
        out_channels_in = model_channels if io_block_channels is None else io_block_channels[0]
        self.out_layer = sp.SparseLinear(out_channels_in, out_channels)
    ...
    def initialize_weights(self) -> None:
        ...
        # Alpha starts as a no-op, so TRELLIS weights keep their original behavior.
    ...
    def forward(
        ...
        t_emb = t_emb + self.alpha_embedder(alpha * 1000.0)
        ...
        if self.separate_cond:
            ...
            cond = (
                cond1.type(self.dtype),
                cond2.type(self.dtype),
                alpha_cond.reshape(x.shape[0]).to(device=x.device, dtype=self.dtype),
            )
        else:
