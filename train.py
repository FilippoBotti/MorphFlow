...
def build_parser():
    ...
    parser.add_argument("--lr_scheduler", type=str, choices=["none", "cosine", "plateau"], default="cosine", help="none keeps a constant LR; cosine steps every batch; plateau warms up then uses validation loss.")
...
def train(args):
    ...
    if resume_ckpt is not None:
        if args.resume_optimizer == 1:
            ...
            sched_state = resume_ckpt.get("scheduler", None)
            if scheduler is not None and sched_state is not None:
                ...
                accelerator.print("No scheduler in this run; skipping scheduler state restore.")
            elif sched_state is None:
                accelerator.print("Checkpoint has no scheduler state; skipping scheduler restore.")
        else:
        ...
        if val_loader is not None and (epoch % max(1, args.val_every) == 0):
            ...
            with torch.no_grad():
                for val_batch_idx, val_batch in enumerate(val_bar, start=1):
                    use_best_extra_losses = args.flow_target in ("ss", "slat")
                    with accelerator.autocast():
                        val_loss = compute_loss(
                            ...
                            symmetry_loss_weight=args.symmetry_loss_weight if use_best_extra_losses else 0.0,
